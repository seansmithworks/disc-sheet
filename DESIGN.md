...
- **Two shadows, one at a time.** The silhouette shadow (`<MorphSheet.Shadow>`) covers the morph. The sheet's own resting shadow appears only after the open settles (`data-morph-sheet-settled`) and drops the instant a close starts.
...
1. **One surface, one clock.** Surface box, silhouette shadow, corner radius and close mask all derive from `collapseProgress`. Nothing has its own spring. A frame where the shadow and the surface disagree is a bug, not a tuning question. *(Known violation: the first painted frame of an open, where the surface leads the shadow by one frame. Open, 2026-09-11.)*
2. **Nothing appears from nothing.** The sheet must never paint at a size the disc did not grow into. A stall that skips the first 30% of the morph is a defect even if every frame after it is perfect. *(Known violation: ~50ms mount stall on open. Open.)*
